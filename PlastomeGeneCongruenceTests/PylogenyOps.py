...
def iqtree_autest(alignment, gene_name):
    log = []
...
def iqtree2_autest(alignment, iqtree2_path, gene_name):
    log = []
    log.append(commandline(iqtree2_path + " -s " + alignment +" -m GTR+I+G -z " + "RAxML_" + gene_name + "_COMBINED.tre -te RAxML_bestTree.withoutConstraints_" + gene_name + " -zb 10000 -au -pre " + gene_name + "_IQTree -quiet"))
    return log
...
def iqtree_mltree(alignment, constraints, gene_name):
    ...
    log.append(commandline("cat " + gene_name + "_IQTree_unconst.treefile " + ''.join([gene_name + "_IQTree_hypo" + str(i) + ".treefile " for i in range(len(constraints))]) + "> " + "IQTree_" + gene_name + "_COMBINED.tre"))
    return log
